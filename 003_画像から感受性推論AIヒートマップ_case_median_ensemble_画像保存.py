...
def save_result_images(
    ...
    source_stem = make_safe_filename_part(image_path.stem)
    class_part = make_safe_filename_part(final_class)
    prefix = f"case_median_{timestamp}_{class_part}_{source_stem}"

    saved_paths = {
        "used_image": OUTPUT_DIR / f"{prefix}_used_image.png",
        "vit_heatmap": OUTPUT_DIR / f"{prefix}_vit_heatmap.png",
        "vit_mixed": OUTPUT_DIR / f"{prefix}_vit_mixed.png",
        "eva_heatmap": OUTPUT_DIR / f"{prefix}_eva_heatmap.png",
        "eva_mixed": OUTPUT_DIR / f"{prefix}_eva_mixed.png",
    }

    orig_img.save(saved_paths["used_image"])
    vit_heat.save(saved_paths["vit_heatmap"])
    vit_overlay.save(saved_paths["vit_mixed"])
    eva_heat.save(saved_paths["eva_heatmap"])
    eva_overlay.save(saved_paths["eva_mixed"])
    return saved_paths
...
def main():
    ...
    saved_paths = save_result_images(
    ...
    log_print("\n===== saved result images =====")
    log_print(f"used_image : {saved_paths['used_image']}")
    log_print(f"vit_heatmap: {saved_paths['vit_heatmap']}")
    log_print(f"vit_mixed  : {saved_paths['vit_mixed']}")
    log_print(f"eva_heatmap: {saved_paths['eva_heatmap']}")
    log_print(f"eva_mixed  : {saved_paths['eva_mixed']}")
