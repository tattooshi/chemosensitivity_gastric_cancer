...
- その後に 616x616 へ変換する
- ViTのpatch tokenを平均してAVG特徴を抽出する
"""
...
ENSEMBLE_INFO_PATH = Path(
    "/home/tatsushi/デスクトップ/val-result/EVA02_val_val-100/eva02_large_patch14_0448.mim_in22k_ft_in22k/Vit_fixed_val_classes_models_0sens_positive/top5_ensemble_info.pth"
)
...
# =========================================================
@torch.no_grad()
def extract_avg_feature(model, x: torch.Tensor) -> torch.Tensor:
...
# =========================================================
# ViT AVG token relevance map
# =========================================================
@torch.no_grad()
def extract_vit_avg_token_relevance_map(model, x: torch.Tensor) -> np.ndarray:
    feats = model.forward_features(x) if hasattr(model, "forward_features") else model(x)
    if feats.ndim != 3 or feats.shape[1] < 2:
        raise RuntimeError("ViTのtoken表現が取得できませんでした。")

    patch_tokens = feats[:, 1:, :]
    avg_token = patch_tokens.mean(dim=1, keepdim=True)

    rel = F.cosine_similarity(
        patch_tokens,
        avg_token.expand_as(patch_tokens),
        dim=-1
    )
...
def show_result_tk(
    ...
    root = tk.Tk()
    root.title("Median AVG Feature Image + Heatmap")
...
def predict_one_image_ensemble(models, feat_model, transform, image_path: Path, device):
    ...
    feat_1d = extract_avg_feature(feat_model, x_img).squeeze(0).float().reshape(-1)
...
def main():
    ...
    log_print("pooling  : avg_patch_tokens")
    ...
    rel_map = extract_vit_avg_token_relevance_map(feat_model, x_img)
