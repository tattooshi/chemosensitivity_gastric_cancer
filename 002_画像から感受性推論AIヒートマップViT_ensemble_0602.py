...
"""
004_画像から感受性推論AIヒートマップ_dual_ensemble.py の ViT-CLS 側だけを使う単独版。

- ViT-Large / 616px / CLS特徴 / top5 ensemble
- 入力画像は中央最大正方形にcropしてから推論
- 判定は ViT ensemble の p(0_sens) と cutoff で行う
- 004 本体は変更しない
"""
...
BASE_PATH = Path(__file__).with_name("009_画像から感受性推論AIヒートマップ.py")
...
def show_result_tk(
    ...
    root = tk.Tk()
    root.title("ViT-CLS Ensemble Result")
    ...
    tk.Label(
        header,
        text=f"ViT-CLS Ensemble Prediction: {pred_class}",
        bg=pred_bg,
        fg="white",
        font=("Arial", 22, "bold"),
        anchor="w",
...
def main():
    ...
    rows = []
    log_print("\n===== per-image ViT ensemble inference =====")
